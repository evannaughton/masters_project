...
results_df <- data.frame(
  Immune_Cell_Type = character(),
  P_Value = numeric(),
  Mean_High_Group = numeric(),
  Mean_Low_Group = numeric(),
  stringsAsFactors = FALSE
)

# Perform t-test for each immune cell type and store the results
for (cell_type in unique(merged_data$Var1)) {
  subset_data <- merged_data %>% filter(Var1 == cell_type)
  
  # Perform the t-test
  t_test_result <- t.test(value ~ group, data = subset_data)
  
  # Calculate means for high and low groups
  mean_high <- mean(subset_data$value[subset_data$group == "High Expression"])
  mean_low <- mean(subset_data$value[subset_data$group == "Low Expression"])
  
  # Append the results to the results_df
  results_df <- rbind(results_df, data.frame(
    Immune_Cell_Type = cell_type,
    P_Value = round(t_test_result$p.value, 4),
    Mean_High_Group = round(mean_high, 4),
    Mean_Low_Group = round(mean_low, 4)
  ))
}

# Convert values to decimal point notation
results_df <- results_df %>%
  mutate(
    P_Value = format(P_Value, scientific = FALSE, digits = 4),
    Mean_High_Group = format(Mean_High_Group, scientific = FALSE, digits = 4),
    Mean_Low_Group = format(Mean_Low_Group, scientific = FALSE, digits = 4)
  )
...
write.csv(results_df, "/home/evannaughton/project/additional_dataframes_if_needed/xcell_immune_cell_abundance_comparison.csv")
```
