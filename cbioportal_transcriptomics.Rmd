...
ggsurvplot(surv_models, data = finaldf, pval = TRUE,
           ...
           title = "Survival Curves by Quartile of MKI67 Expression",
           xlab = "Time (months)", ylab = "Survival Probability",
           risk.table = TRUE, 
...
ggsurvplot(surv_models2, data = finaldf, pval = TRUE,
           legend.title = "Quartile",
           title = "Disease Free Survival Curves by Quartile of MKI67 Expression",
           xlab = "Time (months)", ylab = "Survival Probability",
           risk.table = TRUE, 
           risk.table.title = "Risk Table", 
           risk.table.col = "strata", 
...
ggsurvplot(surv_models3, data = finaldf, pval = TRUE,
           legend.title = "Quartile",
           title = "Disease Specific Survival Curves by Quartile of MKI67 Expression",
           xlab = "Time (months)", ylab = "Survival Probability",
           risk.table = TRUE, 
           risk.table.title = "Risk Table", 
           risk.table.col = "strata", 
...
ggsurvplot(surv_model2, data = quartile_data, pval = TRUE, palette = c("turquoise", "red"),
           ...
           conf.int = TRUE,
           title = "OS Curves for MKI67 Expression Quartiles 1 and 4",
           xlab = "Time (months)", ylab = "Survival Probability", risk.table = TRUE, 
           risk.table.title = "Risk Table", 
           risk.table.col = "strata", 
           tables.height = 0.3, 
           pval.coord = c(-0.3, 0.2),
           theme = theme(plot.margin = margin(t = 2, r = 2, b = 2, l = 2, unit = "cm")),
           size = 0.8) 
...
# Summary of the survival model to extract median survival times
summary_surv1 <- summary(surv_model2)

# Extract median survival times for each group
median_survival1 <- summary_surv1$table[, "median"]

print(median_survival1)
```
```{r}
summary_surv2
```
...
ggsurvplot(surv_model3, data = quartile_data, pval = TRUE, palette = c("turquoise", "red"),
           legend.labs = c("Quartile 1", "Quartile 4"),
           conf.int = TRUE,
           title = "DFS Curves for MKI67 Expression Quartiles 1 and 4",
           xlab = "Time (months)", ylab = "Survival Probability", risk.table = TRUE, 
           risk.table.title = "Risk Table", 
           risk.table.col = "strata", 
           tables.height = 0.3, 
...
# Summary of the survival model to extract median survival times
summary_surv2 <- summary(surv_model3)

# Extract median survival times for each group
median_survival2 <- summary_surv2$table[, "median"]

print(median_survival2)
```
...
ggsurvplot(surv_model4, data = quartile_data, pval = TRUE, palette = c("turquoise", "red"),
           legend.labs = c("Quartile 1", "Quartile 4"),
           conf.int = TRUE,
           title = "DSS Curves for MKI67 Expression Quartiles 1 and 4",
           xlab = "Time (months)", ylab = "Survival Probability", risk.table = TRUE, 
           risk.table.title = "Risk Table", 
           risk.table.col = "strata", 
           tables.height = 0.3, 
...
# Summary of the survival model to extract median survival times
summary_surv3 <- summary(surv_model4)

# Extract median survival times for each group
median_survival3 <- summary_surv3$table[, "median"]

print(median_survival3)
```

```{r}
# Fitting a Multivariable Cox Proportional Hazards Model to the OS survival plot to identify potential confounding variables
quartile_data$quartile <- factor(quartile_data$quartile)

# Create the survival object
surv_cox1 <- Surv(quartile_data$Overall.Survival..Months., quartile_data$OS)

# Fit the Cox Proportional Hazards Model
cox_model1 <- coxph(surv_cox1 ~ quartile + Diagnosis.Age + Mitotane.Therapy + Sex + Neoplasm.Disease.Stage.American.Joint.Committee.on.Cancer.Code + Primary.Tumor.Laterality + Surgical.Margin.Resection.Status, data = quartile_data)
...
# Create the survival object
surv_cox2 <- Surv(quartile_data$Months.of.disease.specific.survival, quartile_data$DSS)

# Fit the Cox Proportional Hazards Model
cox_model2 <- coxph(surv_cox2 ~ quartile + Diagnosis.Age + Mitotane.Therapy + Sex + Neoplasm.Disease.Stage.American.Joint.Committee.on.Cancer.Code + Primary.Tumor.Laterality + Surgical.Margin.Resection.Status, data = quartile_data)
...
```{r}
# Creating a table of the CPHM

# Tidy model output
tidy_cox <- tidy(cox_model1, exponentiate = TRUE, conf.int = TRUE)

tidy_cox %>%
  select(term, hazard_ratio = estimate, conf.low, conf.high, p.value) %>%
  mutate(term = recode(term,
                       "quartile4" = "Quartile 4",
                       "Diagnosis.Age" = "Age",
                       "Mitotane.TherapyYES" = "Mitotane Therapy",
                       "SexMale" = "Gender",
                       "Neoplasm.Disease.Stage.American.Joint.Committee.on.Cancer.CodeStage II" = "Stage II",
                       "Neoplasm.Disease.Stage.American.Joint.Committee.on.Cancer.CodeStage III" = "Stage III",
                       "Neoplasm.Disease.Stage.American.Joint.Committee.on.Cancer.CodeStage IV" = "Stage IV",
                       "Primary.Tumor.LateralityRight" = "Primary Tumor Laterality",
                       "Surgical.Margin.Resection.StatusR1" = "Surgical Margin R1",
                       "Surgical.Margin.Resection.StatusR2" = "Surgical Margin R2",
                       "Surgical.Margin.Resection.StatusRX" = "Surgical Margin RX")) %>%
  kable("html", caption = "Multivariate Cox Proportional Hazards Model Results") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```

The above table looks good, however we wish to create a table that has both OS and DSS multivariable Cox proportional hazards model results.

```{r}
# Extract results from both models
os_results <- summary(cox_model1)
dss_results <- summary(cox_model2)

# Combined results
results_combined <- data.frame(
  Term = rownames(os_results$coefficients),
  `OS HR` = os_results$coefficients[, "exp(coef)"],
  `OS 95% CI Lower` = os_results$conf.int[, "lower .95"],
  `OS 95% CI Upper` = os_results$conf.int[, "upper .95"],
  `OS p-value` = os_results$coefficients[, "Pr(>|z|)"],
  `DSS HR` = dss_results$coefficients[, "exp(coef)"],
  `DSS 95% CI Lower` = dss_results$conf.int[, "lower .95"],
  `DSS 95% CI Upper` = dss_results$conf.int[, "upper .95"],
  `DSS p-value` = dss_results$coefficients[, "Pr(>|z|)"]
)
...
```{r}
# Tidy up the results again & compute confidence intervals
os_conf <- confint(cox_model1)
dss_conf <- confint(cox_model2)

# Convert confidence intervals to data frames
os_conf <- as.data.frame(os_conf)
dss_conf <- as.data.frame(dss_conf)

# Add column names to confidence intervals
colnames(os_conf) <- c("conf.low", "conf.high")
colnames(dss_conf) <- c("conf.low", "conf.high")

# Tidy up the results
os_tidy <- tidy(cox_model1)
dss_tidy <- tidy(cox_model2)

# Combine tidy results with confidence intervals
os_tidy <- cbind(os_tidy, os_conf)
dss_tidy <- cbind(dss_tidy, dss_conf)
...
combined_results <- data.frame(
  Term = os_tidy$term,
  `OS HR` = round(os_tidy$estimate, 3),
  `OS 95% CI Lower` = round(os_tidy$conf.low, 3),
  `OS 95% CI Upper` = round(os_tidy$conf.high, 3),
  `OS p-value` = round(os_tidy$p.value, 4),
  `DSS HR` = round(dss_tidy$estimate, 3),
  `DSS 95% CI Lower` = round(dss_tidy$conf.low, 3),
  `DSS 95% CI Upper` = round(dss_tidy$conf.high, 3),
  `DSS p-value` = round(dss_tidy$p.value, 4)
)
...
gt_table <- gt(combined_results) %>%
  ...
  cols_label(
    Term = "Term",
    `OS.HR` = "OS HR",
    `OS.95..CI.Lower` = "OS 95% CI Lower",
    `OS.95..CI.Upper` = "OS 95% CI Upper",
    `OS.p.value` = "OS p-value",
    `DSS.HR` = "DSS HR",
    `DSS.95..CI.Lower` = "DSS 95% CI Lower",
    `DSS.95..CI.Upper` = "DSS 95% CI Upper",
    `DSS.p.value` = "DSS p-value"
  ) %>%
  tab_spanner(
    label = "Overall Survival",
    columns = vars(`OS.HR`, `OS.95..CI.Lower`, `OS.95..CI.Upper`, `OS.p.value`)
  ) %>%
  tab_spanner(
    label = "Disease-Specific Survival",
    columns = vars(`DSS.HR`, `DSS.95..CI.Lower`, `DSS.95..CI.Upper`, `DSS.p.value`)
  )
...
options(chromote.browser = "/bin/chromium-browser")

# Save the table as an HTML file
gtsave(gt_table, "cox_model_results.html")

# Capture the HTML file as an image
webshot("cox_model_results.html", "cox_model_results.png")
```
